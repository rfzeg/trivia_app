    ...
    def play_quiz():
        ...
        try:
            ...
            if(current_category_id == 0):
                ...
                questions_in_category = db.session.query(Question.id).filter(
                    Question.category == current_category_id).all()

            flat_questions_in_category = [
                j for sub in questions_in_category for j in sub]
            # filter out previous questions
            filtered_questions_in_category = [
                question for question in flat_questions_in_category if question not in previous_questions]
